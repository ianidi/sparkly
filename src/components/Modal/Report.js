...
export default class ReportModal extends React.PureComponent {
  ...
  report = async index => {
    const result = await this.props.feed.Report(index);

    if (result) {
      Alert.alert(
        "Жалоба отправлена",
        "Ваша жалоба будет рассмотрена в течение 24 часов. Мы не извещаем о резульатах рассмотрения. Однако в случае удовлетворения, контент будет удален, а к нарушителю применены меры.",
  ...
  renderContent = () => [
    <View style={s.content__header} key="0">
      <Title>Пожаловаться на контент</Title>
      <Text style={s.content__subheading}>
        Обратите внимание: жалоба — это не дизлайк. Жалуйтесь на контент только
        в том случае, если он нарушает правила сервиса.
      </Text>
    </View>,
